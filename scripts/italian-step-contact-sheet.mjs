...
const output = join(root, 'qa-artifacts', 'italian-review');
await mkdir(output, { recursive: true });

for (let start = 0; start < italianRecipeDrafts.length; start += 7) {
  const recipes = italianRecipeDrafts.slice(start, start + 7);
  const width = 240;
  ...
  const file = join(output, `italian-steps-${Math.floor(start / 7) + 1}.jpg`);
