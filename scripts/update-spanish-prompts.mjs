...
const file = new URL('../docs/illustration-prompts/spanish-prompts.json', import.meta.url);
...
for (const recipe of spanishRecipeDrafts) {
  ...
  if (old) {
    if (old.sceneHash && old.sceneHash !== sceneHash && !refresh.has(recipe.id)) {
      throw new Error(`${recipe.id}: scenes changed; update the prompt and review the affected source explicitly`);
    }
...
console.log(`Spanish prompts: ${record.entries.length} source sheets, ${record.entries.reduce((n, e) => n + e.usedPanels, 0)} planned step images.`);
