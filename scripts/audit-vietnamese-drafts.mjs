...
import { vietnameseRecipeDrafts as recipes } from "../src/vietnamese-recipes.mjs";
import { vietnamesePhotoCandidates as photos } from "../src/vietnamese-photos.mjs";
...
const promptSet = "vietnamese-cooking-steps-v1";
const generatedAt = "2026-09-12";
const hash = (buffer) => createHash("sha256").update(buffer).digest("hex");
const promptRecords = await Promise.all(["a", "bc"].map(async (group) => JSON.parse(await readFile(resolve(root, `docs/illustration-prompts/vietnamese-${group}-prompts.json`), "utf8"))));
...
if (recipes.length < 21) failures.push(`Expected more than 20 Vietnamese recipes; found ${recipes.length}`);
if (recipeIds.size !== recipes.length) failures.push("Duplicate Vietnamese recipe IDs");
if (photosById.size !== photos.length) failures.push("Duplicate Vietnamese photo IDs");
if (photos.length !== recipes.length || photos.some((photo) => !recipeIds.has(photo.id))) failures.push("Vietnamese recipe/photo identity mismatch");
if (new Set(photos.map((photo) => photo.sourceAssetSha256)).size !== photos.length) failures.push("A finished photograph is reused across Vietnamese recipes");
const stepCounts = recipes.map((recipe) => recipe.instructions?.length || 0);
if (new Set(stepCounts).size < 3) failures.push("Methods have lost their natural step-count variation");

for (const recipe of recipes) {
  if (recipe.cuisine !== "vietnamese") failures.push(`${recipe.id}: incorrect cuisine`);
...
let illustrations = [];
try {
  sets = (await import("../src/vietnamese-illustration-sets.mjs")).vietnameseIllustrationSets;
...
if (sets.length !== recipes.length || setsById.size !== sets.length || sets.some((set) => !recipeIds.has(set.recipeId))) failures.push("Vietnamese illustration-set identity mismatch");
...
if (new Set(illustrations.map((entry) => entry.sourceAsset)).size !== illustrations.length || new Set(illustrations.map((entry) => entry.sourceAssetSha256)).size !== illustrations.length) failures.push("A Vietnamese step illustration is reused");
...
if (failures.length) {
  console.error(`Vietnamese draft audit failed (${failures.length}):\n- ${failures.join("\n- ")}`);
  process.exit(1);
}
console.log(`Vietnamese draft audit passed: ${recipes.length} five-language recipes, ${photos.length} licensed real photographs, ${expectedSteps} reviewed hand-free illustrations; content facts, mappings and hashes verified.`);
