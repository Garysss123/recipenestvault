...
const promptFile = join(root, 'docs', 'illustration-prompts', 'african-prompts.json');
const record = JSON.parse(await readFile(promptFile, 'utf8'));
const byId = new Map(record.entries.map((entry) => [entry.recipeId, entry]));
if (record.entries.length !== africanRecipeDrafts.length || byId.size !== record.entries.length) throw new Error('African prompt/recipe identity mismatch');
const requested = new Set(process.argv.slice(2));
const recipes = requested.size ? africanRecipeDrafts.filter((recipe) => requested.has(recipe.id)) : africanRecipeDrafts;
if (requested.size && recipes.length !== requested.size) throw new Error('Unknown African recipe id');
...
for (const recipe of recipes) {
  ...
  const sheetsRoot = resolve(root, 'assets', 'recipes', 'illustration-sheets-generated', 'african');
  ...
  sets.push({
    recipeId: recipe.id,
    promptSet: 'african-cooking-steps-v1',
...
let old = [];
try { old = (await import('../src/african-illustration-sets.mjs')).africanIllustrationSets; } catch {}
...
await writeFile(join(root, 'src', 'african-illustration-sets.mjs'), `// Generated from visually reviewed African ink-and-gouache source sheets and recorded panel layouts.\nexport const africanIllustrationSets = ${JSON.stringify(merged, null, 2)};\n`, 'utf8');
console.log(`African illustration manifest: ${merged.length} sets, ${merged.reduce((sum, set) => sum + set.hashes.length, 0)} distinct method illustrations.`);
