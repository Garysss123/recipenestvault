...
const entries = canadianRecipeDrafts.map((recipe) => {
  const columns = 3;
  const rows = 3;
  ...
  return {
    ...
    style,
    prompt,
    sourceSheet: `assets/recipes/illustration-sheets-generated/canadian/${recipe.id}.png`,
    reviewed: false
  };
});
if (entries.length !== 13 || new Set(entries.map((entry) => entry.recipeId)).size !== 13 || entries.some((entry) => entry.usedPanels > 9)) throw new Error('Canadian illustration prompt identity/grid mismatch');
const record = { promptSet, generatedAt: '2026-09-19', style, entries };
const jsonPath = join(root, 'docs', 'illustration-prompts', 'canadian-prompts.json');
await mkdir(dirname(jsonPath), { recursive: true });
await writeFile(jsonPath, JSON.stringify(record, null, 2) + '\n', 'utf8');
const markdown = [
  '# Canadian recipe step illustration sheets',
  '',
...
const markdownPath = join(root, 'docs', 'illustration-prompts', 'canadian-cooking-steps-v1.md');
await writeFile(markdownPath, markdown, 'utf8');
console.log(`Wrote ${relative(root, jsonPath)} and ${relative(root, markdownPath)} for ${entries.length} Canadian recipe sheets.`);
