...
import { mediterraneanRecipeDrafts } from '../src/mediterranean-recipes.mjs';

const root = dirname(dirname(fileURLToPath(import.meta.url)));
const promptFile = join(root, 'docs', 'illustration-prompts', 'mediterranean-prompts.json');
const record = JSON.parse(await readFile(promptFile, 'utf8'));
const byId = new Map(record.entries.map((entry) => [entry.recipeId, entry]));
if (record.entries.length !== mediterraneanRecipeDrafts.length || byId.size !== record.entries.length) throw new Error('Mediterranean prompt/recipe identity mismatch');
const requested = new Set(process.argv.slice(2));
const recipes = requested.size ? mediterraneanRecipeDrafts.filter((recipe) => requested.has(recipe.id)) : mediterraneanRecipeDrafts;
if (requested.size && recipes.length !== requested.size) throw new Error('Unknown Mediterranean recipe id');
...
for (const recipe of recipes) {
  ...
  const sheetsRoot = resolve(root, 'assets', 'recipes', 'illustration-sheets-generated', 'mediterranean');
  ...
  sets.push({
    recipeId: recipe.id,
    promptSet: 'mediterranean-cooking-steps-v1',
...
let old = [];
try { old = (await import('../src/mediterranean-illustration-sets.mjs')).mediterraneanIllustrationSets; } catch {}
...
await writeFile(join(root, 'src', 'mediterranean-illustration-sets.mjs'), `// Generated from visually reviewed Mediterranean ink-and-gouache source sheets and recorded panel layouts.\nexport const mediterraneanIllustrationSets = ${JSON.stringify(merged, null, 2)};\n`, 'utf8');
console.log(`Mediterranean illustration manifest: ${merged.length} sets, ${merged.reduce((sum, set) => sum + set.hashes.length, 0)} distinct method illustrations.`);
