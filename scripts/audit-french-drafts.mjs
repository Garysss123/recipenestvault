...
import { frenchRecipeDrafts as recipes } from "../src/french-recipes.mjs";

const contentOnly = process.argv.includes('--content-only');
const photos = contentOnly ? [] : (await import('../src/french-photos.mjs')).frenchPhotoCandidates;
...
const promptSet = "french-cooking-steps-v1";
const generatedAt = "2026-09-12";
const hash = (buffer) => createHash("sha256").update(buffer).digest("hex");
const promptRecords = [JSON.parse(await readFile(resolve(root, 'docs/illustration-prompts/french-prompts.json'), 'utf8'))];
...
if (!contentOnly && recipes.length < 21) failures.push(`Expected more than 20 French recipes; found ${recipes.length}`);
if (recipeIds.size !== recipes.length) failures.push("Duplicate French recipe IDs");
if (!contentOnly && photosById.size !== photos.length) failures.push("Duplicate French photo IDs");
if (!contentOnly && (photos.length !== recipes.length || photos.some((photo) => !recipeIds.has(photo.id)))) failures.push("French recipe/photo identity mismatch");
if (!contentOnly && new Set(photos.map((photo) => photo.sourceAssetSha256)).size !== photos.length) failures.push("A finished photograph is reused across French recipes");
...
for (const recipe of recipes) {
  if (recipe.cuisine !== "french") failures.push(`${recipe.id}: incorrect cuisine`);
...
if (contentOnly) {
  ...
  console.log('French content check passed: ' + recipes.length + ' current drafts; all five-language fields and measurable facts verified. Publication assets are not checked in this mode.');
  process.exit(0);
}
...
let illustrations = [];
try {
  sets = (await import("../src/french-illustration-sets.mjs")).frenchIllustrationSets;
...
if (sets.length !== recipes.length || setsById.size !== sets.length || sets.some((set) => !recipeIds.has(set.recipeId))) failures.push("French illustration-set identity mismatch");
...
if (new Set(illustrations.map((entry) => entry.sourceAsset)).size !== illustrations.length || new Set(illustrations.map((entry) => entry.sourceAssetSha256)).size !== illustrations.length) failures.push("A French step illustration is reused");
...
if (failures.length) {
  console.error(`French draft audit failed (${failures.length}):\n- ${failures.join("\n- ")}`);
  process.exit(1);
}
console.log(`French draft audit passed: ${recipes.length} five-language recipes, ${photos.length} licensed real photographs, ${expectedSteps} reviewed hand-free illustrations; content facts, mappings and hashes verified.`);
