...
const directory = join(root, 'assets', 'recipes', 'alternatives', 'african');
await mkdir(directory, { recursive: true });
const [command, id, ...words] = process.argv.slice(2);
const headers = { 'User-Agent': 'RecipeNestVault/0.1 (African licensed photo research; recipenestvault.com)' };
...
if (command === 'search') {
  if (!id || !words.length || !/^[-a-z0-9]+$/.test(id)) throw new Error('Usage: research-african-photos.mjs search <id> <query>');
  ...
  if (!id || !words.length || !/^[-a-z0-9]+$/.test(id)) throw new Error('Usage: research-african-photos.mjs download <id> <File:title>');
  ...
  throw new Error('Usage: research-african-photos.mjs search|download <id> <query or File:title>');
}
