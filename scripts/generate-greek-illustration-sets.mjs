...
import { greekRecipeDrafts } from '../src/greek-recipes.mjs';

const root = dirname(dirname(fileURLToPath(import.meta.url)));
const promptFile = join(root, 'docs', 'illustration-prompts', 'greek-prompts.json');
const record = JSON.parse(await readFile(promptFile, 'utf8'));
const entries = record.entries;
const byId = new Map(entries.map(entry => [entry.recipeId, entry]));
if (entries.length !== greekRecipeDrafts.length || byId.size !== entries.length) throw new Error('Greek prompt/recipe identity count mismatch');

const requested = new Set(process.argv.slice(2));
const recipes = requested.size ? greekRecipeDrafts.filter(recipe => requested.has(recipe.id)) : greekRecipeDrafts;
if (requested.size && recipes.length !== requested.size) throw new Error('Unknown Greek recipe id');
...
for (const recipe of recipes) {
  ...
  const sheetsRoot = resolve(root, 'assets', 'recipes', 'illustration-sheets-generated', 'greek');
  ...
  sets.push({
    recipeId: recipe.id,
    promptSet: 'greek-cooking-steps-v1',
...
if (requested.size) {
  let old = [];
  try {
    old = (await import('../src/greek-illustration-sets.mjs')).greekIllustrationSets;
  } catch {
    old = [];
  }
...
await writeFile(join(root, 'src', 'greek-illustration-sets.mjs'), `// Generated from visually reviewed hand-free Greek source sheets and recorded panel layouts.\nexport const greekIllustrationSets = ${JSON.stringify(sets, null, 2)};\n`, 'utf8');
console.log(`Greek illustration manifest: ${sets.length} sets, ${sets.reduce((sum, set) => sum + set.hashes.length, 0)} separate step images.`);
