...
if (entries.length !== frenchRecipeDrafts.length || byId.size !== entries.length) throw new Error('French prompt/recipe identity count mismatch');

const requested = new Set(process.argv.slice(2));
const recipes = requested.size ? frenchRecipeDrafts.filter(recipe => requested.has(recipe.id)) : frenchRecipeDrafts;
if (requested.size && recipes.length !== requested.size) throw new Error('Unknown French recipe id');
...
for (const recipe of recipes) {
  ...
  const sheetsRoot = resolve(root, 'assets', 'recipes', 'illustration-sheets-generated', 'french');
  ...
  sets.push({
    recipeId: recipe.id,
    promptSet: 'french-cooking-steps-v1',
...
if (requested.size) {
  const old = (await import('../src/french-illustration-sets.mjs')).frenchIllustrationSets;
  const updated = new Map(sets.map(set => [set.recipeId, set]));
  sets.splice(0, sets.length, ...old.map(set => updated.get(set.recipeId) || set));
}

await writeFile(join(root, 'src', 'french-illustration-sets.mjs'), `// Generated from visually reviewed hand-free French source sheets and recorded panel layouts.\nexport const frenchIllustrationSets = ${JSON.stringify(sets, null, 2)};\n`, 'utf8');
console.log(`French illustration manifest: ${sets.length} sets, ${sets.reduce((sum, set) => sum + set.hashes.length, 0)} separate step images.`);
