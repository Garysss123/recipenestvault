...
const directory = join(root, 'assets', 'recipes', 'alternatives', 'middle-eastern');
await mkdir(directory, { recursive: true });
const [command, id, ...words] = process.argv.slice(2);
const headers = { 'User-Agent': 'RecipeNestVault/0.1 (Middle Eastern licensed photo research; recipenestvault.com)' };
...
if (command === 'search') {
  if (!id || !words.length || !/^[-a-z0-9]+$/.test(id)) throw new Error('Usage: research-middle-eastern-photos.mjs search <id> <query>');
  ...
  if (!id || !words.length || !/^[-a-z0-9]+$/.test(id)) throw new Error('Usage: research-middle-eastern-photos.mjs download <id> <File:title>');
  ...
  if (info.width > 1280) {
    ...
    if (Math.floor(info.height * thumbWidth / info.width) >= 800) {
      imageUrl = new URL(sourceUrl); imageUrl.host = 'thumb.wikimedia.org';
  ...
  throw new Error('Usage: research-middle-eastern-photos.mjs search|download <id> <query or File:title>');
}
