...
const americanRecipeIds = [
  "cheeseburger", "buffalo-wings", "southern-fried-chicken", "memphis-style-ribs", "baked-mac-and-cheese",
  "new-england-clam-chowder", "philly-cheesesteak", "chicken-and-sausage-gumbo", "jambalaya", "biscuits-and-gravy",
  "meatloaf", "skillet-cornbread", "new-york-cheesecake", "apple-pie", "chocolate-chip-cookies"
];
async function assertAmericanCollection(page, locale) {
  const links = await page.locator(".collection-recipe-card > a").evaluateAll((items) => items.map((item) => item.getAttribute("href")).sort());
  const expected = americanRecipeIds.map((id) => `/${locale}/recipes/${id}/`).sort();
  if (JSON.stringify(links) !== JSON.stringify(expected)) throw new Error(`American ${locale} collection does not contain the expected 15 unique recipe routes`);
  if (await page.locator(".collection-recipe-card").count() !== 15) throw new Error("American collection must show 15 cards");
  const broken = await page.locator(".collection-recipe-card img").evaluateAll((images) => images.filter((image) => !image.complete || !image.naturalWidth).length);
  if (broken) throw new Error(`${broken} American card photographs failed to load`);
  if (!(await page.locator("body").evaluate((element) => element.classList.contains("cuisine-american")))) throw new Error("American collection is missing its cuisine theme class");
}
async function assertAmericanRecipe(page, expectedSteps, locale, id) {
  await assertIllustratedRecipe(page, expectedSteps, { traditionalChinese: locale === "zh-hant" });
  const steps = page.locator(".method-section ol > li");
  if (await steps.count() !== expectedSteps) throw new Error(`American ${id} method count differs from its ${expectedSteps} illustrations`);
  for (const entry of await steps.all()) if (await entry.locator(".recipe-step-illustration").count() !== 1) throw new Error(`American ${id} needs exactly one illustration in each step`);
  const images = await page.locator(".recipe-step-illustration img").evaluateAll((entries) => entries.map((entry) => entry.getAttribute("src")));
  if (new Set(images).size !== expectedSteps) throw new Error(`American ${id} reuses a step image`);
  if (await page.locator(`.breadcrumbs a[href="/${locale}/cuisines/american/"]`).count() !== 1) throw new Error(`American ${id} breadcrumb does not return to American cuisine`);
  const expectedRelatedTitle = ({
    en: "More American recipes",
    "zh-hant": "更多美國料理",
    ja: "ほかのアメリカ料理",
    ko: "다른 미국 요리",
    th: "สูตรอาหารอเมริกันเพิ่มเติม"
  })[locale];
  if (await page.locator("#related-title").innerText() !== expectedRelatedTitle) throw new Error(`American ${id} related-recipes title is not cuisine-specific`);
  const relatedLinks = await page.locator(".related-recipes .collection-recipe-card > a").evaluateAll((items) => items.map((item) => item.getAttribute("href")));
  if (relatedLinks.length !== 3 || relatedLinks.some((href) => !href?.startsWith(`/${locale}/recipes/`))) throw new Error(`American ${id} related-recipes links are invalid`);
  for (const target of ["en", "zh-hant", "ja", "ko", "th"]) {
    if (await page.locator(`.language-popover a[href="/${target}/recipes/${id}/"]`).count() !== 1) throw new Error(`American ${id} is missing its ${target} language route`);
  }
}
await inspect({
  name: "en-search-american-desktop", path: "/en/search/?q=cheeseburger", viewport: { width: 1280, height: 900 },
  interact: async (page) => {
    const result = page.locator('.result-card[href="/en/recipes/cheeseburger/"]');
    await result.waitFor({ state: "visible" });
    if (!/American recipe/i.test(await result.innerText())) throw new Error("American search result lacks its cuisine label");
  }
});
for (const [locale, label, viewport] of [
  ["en", "desktop", { width: 1440, height: 1000 }],
  ...
  await inspect({
    name: `${locale}-american-collection-${label}`, path: `/${locale}/cuisines/american/`, viewport,
    interact: async (page) => {
      await assertAmericanCollection(page, locale);
      const response = await page.reload({ waitUntil: "networkidle", timeout: 30000 });
      if (response?.status() !== 200) throw new Error(`American ${locale} direct refresh failed`);
    },
    extraScreenshots: locale === "en" || locale === "zh-hant" ? [
      { name: `${locale}-american-collection-hero-${label}`, selector: ".cuisine-hero" },
      { name: `${locale}-american-first-card-${label}`, selector: ".collection-recipe-card:first-child" }
    ] : []
  });
}
for (const [locale, id, count, label] of [
  ["zh-hant", "cheeseburger", 7, "mobile"],
  ["en", "chicken-and-sausage-gumbo", 12, "desktop"],
  ["ja", "apple-pie", 14, "desktop"],
  ["ko", "skillet-cornbread", 6, "mobile"],
  ["th", "new-england-clam-chowder", 9, "mobile"]
]) {
  await inspect({
    name: `${locale}-american-${id}-${label}`, path: `/${locale}/recipes/${id}/`,
    viewport: label === "mobile" ? { width: 390, height: 844 } : { width: 1366, height: 900 },
    interact: async (page) => assertAmericanRecipe(page, count, locale, id),
    extraScreenshots: [
      { name: `${locale}-american-${id}-hero-${label}`, selector: ".recipe-detail-hero" },
      { name: `${locale}-american-${id}-first-step-${label}`, selector: ".method-section li:first-child" }
    ]
  });
}
await inspect({
  name: "american-language-choice-mobile", path: "/en/recipes/apple-pie/", viewport: { width: 390, height: 844 },
  fullPage: false, suppressLanguagePrompt: false, loadLazyImages: false,
  interact: async (page) => {
    await page.locator('[data-language-choice="zh-hant"]').click();
    await page.waitForLoadState("networkidle");
    if (new URL(page.url()).pathname !== "/zh-hant/recipes/apple-pie/") throw new Error("American language selection lost the recipe route");
    const response = await page.reload({ waitUntil: "networkidle" });
    if (response?.status() !== 200 || await page.locator("[data-language-prompt]").isVisible()) throw new Error("American language preference or deep refresh failed");
  }
});
...
const brazilianRecipeIds = [
  "feijoada", "moqueca-baiana", "pao-de-queijo", "coxinha", "acaraje", "churrasco-picanha",
  "bobo-de-camarao", "vatapa", "escondidinho-de-carne-seca", "arroz-carreteiro", "feijao-tropeiro",
  "brigadeiro", "quindim"
];
async function assertBrazilianCollection(page, locale) {
  const links = await page.locator(".collection-recipe-card > a").evaluateAll((items) => items.map((item) => item.getAttribute("href")).sort());
  const expected = brazilianRecipeIds.map((id) => `/${locale}/recipes/${id}/`).sort();
  if (JSON.stringify(links) !== JSON.stringify(expected)) throw new Error(`Brazilian ${locale} collection does not contain the expected 13 unique recipe routes`);
  if (await page.locator(".collection-recipe-card").count() !== 13) throw new Error("Brazilian collection must show 13 cards");
  const broken = await page.locator(".collection-recipe-card img").evaluateAll((images) => images.filter((image) => !image.complete || !image.naturalWidth).length);
  if (broken) throw new Error(`${broken} Brazilian card photographs failed to load`);
  if (!(await page.locator("body").evaluate((element) => element.classList.contains("cuisine-brazilian")))) throw new Error("Brazilian collection is missing its cuisine theme class");
}
async function assertBrazilianRecipe(page, expectedSteps, locale, id) {
  await assertIllustratedRecipe(page, expectedSteps, { traditionalChinese: locale === "zh-hant" });
  const steps = page.locator(".method-section ol > li");
  if (await steps.count() !== expectedSteps) throw new Error(`Brazilian ${id} method count differs from its ${expectedSteps} illustrations`);
  for (const entry of await steps.all()) if (await entry.locator(".recipe-step-illustration").count() !== 1) throw new Error(`Brazilian ${id} needs exactly one illustration in each step`);
  const images = await page.locator(".recipe-step-illustration img").evaluateAll((entries) => entries.map((entry) => entry.getAttribute("src")));
  if (new Set(images).size !== expectedSteps) throw new Error(`Brazilian ${id} reuses a step image`);
  if (await page.locator(`.breadcrumbs a[href="/${locale}/cuisines/brazilian/"]`).count() !== 1) throw new Error(`Brazilian ${id} breadcrumb does not return to Brazilian cuisine`);
  const expectedRelatedTitle = ({
    en: "More Brazilian recipes",
    "zh-hant": "更多巴西料理",
    ja: "ほかのブラジル料理",
    ko: "다른 브라질 요리",
    th: "สูตรอาหารบราซิลเพิ่มเติม"
  })[locale];
  if (await page.locator("#related-title").innerText() !== expectedRelatedTitle) throw new Error(`Brazilian ${id} related-recipes title is not cuisine-specific`);
  const relatedLinks = await page.locator(".related-recipes .collection-recipe-card > a").evaluateAll((items) => items.map((item) => item.getAttribute("href")));
  if (relatedLinks.length !== 3 || relatedLinks.some((href) => !href?.startsWith(`/${locale}/recipes/`))) throw new Error(`Brazilian ${id} related-recipes links are invalid`);
  for (const target of ["en", "zh-hant", "ja", "ko", "th"]) {
    if (await page.locator(`.language-popover a[href="/${target}/recipes/${id}/"]`).count() !== 1) throw new Error(`Brazilian ${id} is missing its ${target} language route`);
  }
}
await inspect({
  name: "en-search-brazilian-desktop", path: "/en/search/?q=feijoada", viewport: { width: 1280, height: 900 },
  interact: async (page) => {
    const result = page.locator('.result-card[href="/en/recipes/feijoada/"]');
    await result.waitFor({ state: "visible" });
    if (!/Brazilian recipe/i.test(await result.innerText())) throw new Error("Brazilian search result lacks its cuisine label");
  }
});
for (const [locale, label, viewport] of [
  ["en", "desktop", { width: 1440, height: 1000 }],
  ...
  await inspect({
    name: `${locale}-brazilian-collection-${label}`, path: `/${locale}/cuisines/brazilian/`, viewport,
    interact: async (page) => {
      await assertBrazilianCollection(page, locale);
      const response = await page.reload({ waitUntil: "networkidle", timeout: 30000 });
      if (response?.status() !== 200) throw new Error(`Brazilian ${locale} direct refresh failed`);
    },
    extraScreenshots: locale === "en" || locale === "zh-hant" ? [
      { name: `${locale}-brazilian-collection-hero-${label}`, selector: ".cuisine-hero" },
      { name: `${locale}-brazilian-first-card-${label}`, selector: ".collection-recipe-card:first-child" }
    ] : []
  });
}
for (const [locale, id, count, label] of [
  ["zh-hant", "feijoada", 8, "mobile"],
  ["en", "moqueca-baiana", 7, "desktop"],
  ["ja", "coxinha", 10, "desktop"],
  ["ko", "brigadeiro", 6, "mobile"],
  ["th", "quindim", 7, "mobile"]
]) {
  await inspect({
    name: `${locale}-brazilian-${id}-${label}`, path: `/${locale}/recipes/${id}/`,
    viewport: label === "mobile" ? { width: 390, height: 844 } : { width: 1366, height: 900 },
    interact: async (page) => assertBrazilianRecipe(page, count, locale, id),
    extraScreenshots: [
      { name: `${locale}-brazilian-${id}-hero-${label}`, selector: ".recipe-detail-hero" },
      { name: `${locale}-brazilian-${id}-first-step-${label}`, selector: ".method-section li:first-child" }
    ]
  });
}
await inspect({
  name: "brazilian-language-choice-mobile", path: "/en/recipes/quindim/", viewport: { width: 390, height: 844 },
  fullPage: false, suppressLanguagePrompt: false, loadLazyImages: false,
  interact: async (page) => {
    await page.locator('[data-language-choice="zh-hant"]').click();
    await page.waitForLoadState("networkidle");
    if (new URL(page.url()).pathname !== "/zh-hant/recipes/quindim/") throw new Error("Brazilian language selection lost the recipe route");
    const response = await page.reload({ waitUntil: "networkidle" });
    if (response?.status() !== 200 || await page.locator("[data-language-prompt]").isVisible()) throw new Error("Brazilian language preference or deep refresh failed");
  }
});

const canadianRecipeIds = [
  "poutine", "tourtiere", "montreal-smoked-meat-sandwich", "montreal-style-bagels", "butter-tarts",
  "nanaimo-bars", "peameal-bacon-sandwich", "halifax-donair", "jiggs-dinner", "toutons",
  "saskatoon-berry-pie", "maple-taffy", "campfire-bannock"
];
async function assertCanadianCollection(page, locale) {
  const links = await page.locator(".collection-recipe-card > a").evaluateAll((items) => items.map((item) => item.getAttribute("href")).sort());
  const expected = canadianRecipeIds.map((id) => `/${locale}/recipes/${id}/`).sort();
  if (JSON.stringify(links) !== JSON.stringify(expected)) throw new Error(`Canadian ${locale} collection does not contain the expected 13 unique recipe routes`);
  if (await page.locator(".collection-recipe-card").count() !== 13) throw new Error("Canadian collection must show 13 cards");
  const broken = await page.locator(".collection-recipe-card img").evaluateAll((images) => images.filter((image) => !image.complete || !image.naturalWidth).length);
  if (broken) throw new Error(`${broken} Canadian card photographs failed to load`);
  if (!(await page.locator("body").evaluate((element) => element.classList.contains("cuisine-canadian")))) throw new Error("Canadian collection is missing its cuisine theme class");
}
async function assertCanadianRecipe(page, expectedSteps, locale, id) {
  await assertIllustratedRecipe(page, expectedSteps, { traditionalChinese: locale === "zh-hant" });
  const steps = page.locator(".method-section ol > li");
  if (await steps.count() !== expectedSteps) throw new Error(`Canadian ${id} method count differs from its ${expectedSteps} illustrations`);
  for (const entry of await steps.all()) if (await entry.locator(".recipe-step-illustration").count() !== 1) throw new Error(`Canadian ${id} needs exactly one illustration in each step`);
  const images = await page.locator(".recipe-step-illustration img").evaluateAll((entries) => entries.map((entry) => entry.getAttribute("src")));
  if (new Set(images).size !== expectedSteps) throw new Error(`Canadian ${id} reuses a step image`);
  if (await page.locator(`.breadcrumbs a[href="/${locale}/cuisines/canadian/"]`).count() !== 1) throw new Error(`Canadian ${id} breadcrumb does not return to Canadian cuisine`);
  const expectedRelatedTitle = ({
    en: "More Canadian recipes",
    "zh-hant": "更多加拿大料理",
    ja: "ほかのカナダ料理",
    ko: "다른 캐나다 요리",
    th: "สูตรอาหารแคนาดาเพิ่มเติม"
  })[locale];
  if (await page.locator("#related-title").innerText() !== expectedRelatedTitle) throw new Error(`Canadian ${id} related-recipes title is not cuisine-specific`);
  const relatedLinks = await page.locator(".related-recipes .collection-recipe-card > a").evaluateAll((items) => items.map((item) => item.getAttribute("href")));
  if (relatedLinks.length !== 3 || relatedLinks.some((href) => !href?.startsWith(`/${locale}/recipes/`))) throw new Error(`Canadian ${id} related-recipes links are invalid`);
  for (const target of ["en", "zh-hant", "ja", "ko", "th"]) {
    if (await page.locator(`.language-popover a[href="/${target}/recipes/${id}/"]`).count() !== 1) throw new Error(`Canadian ${id} is missing its ${target} language route`);
  }
}
await inspect({
  name: "en-search-canadian-desktop", path: "/en/search/?q=poutine", viewport: { width: 1280, height: 900 },
  interact: async (page) => {
    const result = page.locator('.result-card[href="/en/recipes/poutine/"]');
    await result.waitFor({ state: "visible" });
    if (!/Canadian recipe/i.test(await result.innerText())) throw new Error("Canadian search result lacks its cuisine label");
  }
});
for (const [locale, label, viewport] of [
  ["en", "desktop", { width: 1440, height: 1000 }],
  ...
  await inspect({
    name: `${locale}-canadian-collection-${label}`, path: `/${locale}/cuisines/canadian/`, viewport,
    interact: async (page) => {
      await assertCanadianCollection(page, locale);
      const response = await page.reload({ waitUntil: "networkidle", timeout: 30000 });
      if (response?.status() !== 200) throw new Error(`Canadian ${locale} direct refresh failed`);
    },
    extraScreenshots: locale === "en" || locale === "zh-hant" ? [
      { name: `${locale}-canadian-collection-hero-${label}`, selector: ".cuisine-hero" },
      { name: `${locale}-canadian-first-card-${label}`, selector: ".collection-recipe-card:first-child" }
    ] : []
  });
}
for (const [locale, id, count, label] of [
  ["zh-hant", "poutine", 7, "mobile"],
  ["en", "tourtiere", 8, "desktop"],
  ["ja", "montreal-style-bagels", 9, "desktop"],
  ["ko", "maple-taffy", 5, "mobile"],
  ["th", "campfire-bannock", 6, "mobile"]
]) {
  await inspect({
    name: `${locale}-canadian-${id}-${label}`, path: `/${locale}/recipes/${id}/`,
    viewport: label === "mobile" ? { width: 390, height: 844 } : { width: 1366, height: 900 },
    interact: async (page) => assertCanadianRecipe(page, count, locale, id),
    extraScreenshots: [
      { name: `${locale}-canadian-${id}-hero-${label}`, selector: ".recipe-detail-hero" },
      { name: `${locale}-canadian-${id}-first-step-${label}`, selector: ".method-section li:first-child" }
    ]
  });
}
await inspect({
  name: "canadian-language-choice-mobile", path: "/en/recipes/maple-taffy/", viewport: { width: 390, height: 844 },
  fullPage: false, suppressLanguagePrompt: false, loadLazyImages: false,
  interact: async (page) => {
    await page.locator('[data-language-choice="zh-hant"]').click();
    await page.waitForLoadState("networkidle");
    if (new URL(page.url()).pathname !== "/zh-hant/recipes/maple-taffy/") throw new Error("Canadian language selection lost the recipe route");
    const response = await page.reload({ waitUntil: "networkidle" });
    if (response?.status() !== 200 || await page.locator("[data-language-prompt]").isVisible()) throw new Error("Canadian language preference or deep refresh failed");
  }
});
...
async function assertMediterraneanCollection(page, locale) {
  const links = await page.locator(".collection-recipe-card > a").evaluateAll((items) => items.map((item) => item.getAttribute("href")).sort());
  const expected = mediterraneanRecipeIds.map((id) => `/${locale}/recipes/${id}/`).sort();
  if (JSON.stringify(links) !== JSON.stringify(expected)) throw new Error(`Mediterranean ${locale} collection does not contain the expected 13 unique recipe routes`);
  if (await page.locator(".collection-recipe-card").count() !== 13) throw new Error("Mediterranean collection must show 13 cards");
  const broken = await page.locator(".collection-recipe-card img").evaluateAll((images) => images.filter((image) => !image.complete || !image.naturalWidth).length);
  if (broken) throw new Error(`${broken} Mediterranean card photographs failed to load`);
  if (!(await page.locator("body").evaluate((element) => element.classList.contains("cuisine-mediterranean")))) throw new Error("Mediterranean collection is missing its cuisine theme class");
}
async function assertMediterraneanRecipe(page, expectedSteps, locale, id) {
  await assertIllustratedRecipe(page, expectedSteps, { traditionalChinese: locale === "zh-hant" });
  const steps = page.locator(".method-section ol > li");
  if (await steps.count() !== expectedSteps) throw new Error(`Mediterranean ${id} method count differs from its ${expectedSteps} illustrations`);
  for (const entry of await steps.all()) if (await entry.locator(".recipe-step-illustration").count() !== 1) throw new Error(`Mediterranean ${id} needs exactly one illustration in each step`);
  const images = await page.locator(".recipe-step-illustration img").evaluateAll((entries) => entries.map((entry) => entry.getAttribute("src")));
  if (new Set(images).size !== expectedSteps) throw new Error(`Mediterranean ${id} reuses a step image`);
  if (await page.locator(`.breadcrumbs a[href="/${locale}/cuisines/mediterranean/"]`).count() !== 1) throw new Error(`Mediterranean ${id} breadcrumb does not return to its cuisine collection`);
  const expectedRelatedTitle = ({
    en: "More Mediterranean recipes",
    "zh-hant": "更多地中海料理",
    ja: "ほかの地中海料理",
    ko: "다른 지중해 요리",
    th: "สูตรอาหารเมดิเตอร์เรเนียนเพิ่มเติม"
  })[locale];
  if (await page.locator("#related-title").innerText() !== expectedRelatedTitle) throw new Error(`Mediterranean ${id} related-recipes title is not cuisine-specific`);
  const relatedLinks = await page.locator(".related-recipes .collection-recipe-card > a").evaluateAll((items) => items.map((item) => item.getAttribute("href")));
  if (relatedLinks.length !== 3 || relatedLinks.some((href) => !href?.startsWith(`/${locale}/recipes/`))) throw new Error(`Mediterranean ${id} related-recipes links are invalid`);
  for (const target of ["en", "zh-hant", "ja", "ko", "th"]) {
    if (await page.locator(`.language-popover a[href="/${target}/recipes/${id}/"]`).count() !== 1) throw new Error(`Mediterranean ${id} is missing its ${target} language route`);
  }
}
await inspect({
  name: "en-search-mediterranean-desktop", path: "/en/search/?q=bouillabaisse", viewport: { width: 1280, height: 900 },
  interact: async (page) => {
    const result = page.locator('.result-card[href="/en/recipes/bouillabaisse/"]');
    await result.waitFor({ state: "visible" });
    if (!/Mediterranean recipe/i.test(await result.innerText())) throw new Error("Mediterranean search result lacks its cuisine label");
  }
});
for (const [locale, label, viewport] of [
  ["en", "desktop", { width: 1440, height: 1000 }],
  ...
  await inspect({
    name: `${locale}-mediterranean-collection-${label}`, path: `/${locale}/cuisines/mediterranean/`, viewport,
    interact: async (page) => {
      await assertMediterraneanCollection(page, locale);
      const response = await page.reload({ waitUntil: "networkidle", timeout: 30000 });
      if (response?.status() !== 200) throw new Error(`Mediterranean ${locale} direct refresh failed`);
    },
    extraScreenshots: locale === "en" || locale === "zh-hant" ? [
      { name: `${locale}-mediterranean-collection-hero-${label}`, selector: ".cuisine-hero" },
      { name: `${locale}-mediterranean-first-card-${label}`, selector: ".collection-recipe-card:first-child" }
    ] : []
  });
}
for (const [locale, id, count, label] of [
  ["zh-hant", "bouillabaisse", 6, "mobile"],
  ["en", "caponata", 6, "desktop"],
  ["ja", "crni-rizot", 7, "desktop"],
  ["ko", "brik-a-loeuf", 5, "mobile"],
  ["th", "stuffat-tal-fenek", 7, "mobile"]
]) {
  await inspect({
    name: `${locale}-mediterranean-${id}-${label}`, path: `/${locale}/recipes/${id}/`,
    viewport: label === "mobile" ? { width: 390, height: 844 } : { width: 1366, height: 900 },
    interact: async (page) => assertMediterraneanRecipe(page, count, locale, id),
    extraScreenshots: [
      { name: `${locale}-mediterranean-${id}-hero-${label}`, selector: ".recipe-detail-hero" },
      { name: `${locale}-mediterranean-${id}-first-step-${label}`, selector: ".method-section li:first-child" }
    ]
  });
}
await inspect({
  name: "mediterranean-language-choice-mobile", path: "/en/recipes/pissaladiere/", viewport: { width: 390, height: 844 },
  fullPage: false, suppressLanguagePrompt: false, loadLazyImages: false,
  interact: async (page) => {
    await page.locator('[data-language-choice="zh-hant"]').click();
    await page.waitForLoadState("networkidle");
    if (new URL(page.url()).pathname !== "/zh-hant/recipes/pissaladiere/") throw new Error("Mediterranean language selection lost the recipe route");
    const response = await page.reload({ waitUntil: "networkidle" });
    if (response?.status() !== 200 || await page.locator("[data-language-prompt]").isVisible()) throw new Error("Mediterranean language preference or deep refresh failed");
  }
});
