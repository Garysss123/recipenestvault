...
const entries = otherWorldRecipeDrafts.map((recipe) => {
  const columns = 3;
  const rows = 3;
  ...
  return {
    ...
    style,
    prompt,
    sourceSheet: `assets/recipes/illustration-sheets-generated/other-world/${recipe.id}.png`,
    reviewed: false
  };
});
if (entries.length !== 20 || new Set(entries.map((entry) => entry.recipeId)).size !== 20 || entries.some((entry) => entry.usedPanels > 9)) throw new Error('Other-world illustration prompt identity/grid mismatch');
const record = { promptSet, generatedAt: '2026-09-20', style, entries };
const jsonPath = join(root, 'docs', 'illustration-prompts', 'other-world-prompts.json');
await mkdir(dirname(jsonPath), { recursive: true });
await writeFile(jsonPath, JSON.stringify(record, null, 2) + '\n', 'utf8');
const markdown = [
  '# Other-world recipe step illustration sheets',
  '',
...
const markdownPath = join(root, 'docs', 'illustration-prompts', 'other-world-cooking-steps-v1.md');
await writeFile(markdownPath, markdown, 'utf8');
console.log(`Wrote ${relative(root, jsonPath)} and ${relative(root, markdownPath)} for ${entries.length} Other-world recipe sheets.`);
