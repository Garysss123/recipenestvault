...
const entries = mediterraneanRecipeDrafts.map((recipe) => {
  const columns = 3;
  const rows = 3;
  ...
  return {
    ...
    style,
    prompt,
    sourceSheet: `assets/recipes/illustration-sheets-generated/mediterranean/${recipe.id}.png`,
    reviewed: false
  };
});
if (entries.length !== 13 || new Set(entries.map((entry) => entry.recipeId)).size !== 13 || entries.some((entry) => entry.usedPanels > 9)) throw new Error('Mediterranean illustration prompt identity/grid mismatch');
const record = { promptSet, generatedAt: '2026-09-20', style, entries };
const jsonPath = join(root, 'docs', 'illustration-prompts', 'mediterranean-prompts.json');
await mkdir(dirname(jsonPath), { recursive: true });
await writeFile(jsonPath, JSON.stringify(record, null, 2) + '\n', 'utf8');
const markdown = [
  '# Mediterranean recipe step illustration sheets',
  '',
...
const markdownPath = join(root, 'docs', 'illustration-prompts', 'mediterranean-cooking-steps-v1.md');
await writeFile(markdownPath, markdown, 'utf8');
console.log(`Wrote ${relative(root, jsonPath)} and ${relative(root, markdownPath)} for ${entries.length} Mediterranean recipe sheets.`);
