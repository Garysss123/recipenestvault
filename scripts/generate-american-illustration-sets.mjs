...
import { americanRecipeDrafts } from '../src/american-recipes.mjs';

const root = dirname(dirname(fileURLToPath(import.meta.url)));
const promptFile = join(root, 'docs', 'illustration-prompts', 'american-prompts.json');
const record = JSON.parse(await readFile(promptFile, 'utf8'));
const entries = record.entries;
const byId = new Map(entries.map(entry => [entry.recipeId, entry]));
if (entries.length !== americanRecipeDrafts.length || byId.size !== entries.length) throw new Error('American prompt/recipe identity count mismatch');

const requested = new Set(process.argv.slice(2));
const recipes = requested.size ? americanRecipeDrafts.filter(recipe => requested.has(recipe.id)) : americanRecipeDrafts;
if (requested.size && recipes.length !== requested.size) throw new Error('Unknown American recipe id');
...
for (const recipe of recipes) {
  ...
  const sheetsRoot = resolve(root, 'assets', 'recipes', 'illustration-sheets-generated', 'american');
  ...
  sets.push({
    recipeId: recipe.id,
    promptSet: 'american-cooking-steps-v1',
...
if (requested.size) {
  let old = [];
  try {
    old = (await import('../src/american-illustration-sets.mjs')).americanIllustrationSets;
  } catch {
    old = [];
  }
...
await writeFile(join(root, 'src', 'american-illustration-sets.mjs'), `// Generated from visually reviewed hand-free American source sheets and recorded panel layouts.\nexport const americanIllustrationSets = ${JSON.stringify(sets, null, 2)};\n`, 'utf8');
console.log(`American illustration manifest: ${sets.length} sets, ${sets.reduce((sum, set) => sum + set.hashes.length, 0)} separate step images.`);
