...
import { italianRecipeDrafts } from '../src/italian-recipes.mjs';

const root = dirname(dirname(fileURLToPath(import.meta.url)));
const promptFile = join(root, 'docs', 'illustration-prompts', 'italian-prompts.json');
const record = JSON.parse(await readFile(promptFile, 'utf8'));
const entries = record.entries;
const byId = new Map(entries.map(entry => [entry.recipeId, entry]));
if (entries.length !== italianRecipeDrafts.length || byId.size !== entries.length) throw new Error('Italian prompt/recipe identity count mismatch');

const requested = new Set(process.argv.slice(2));
const recipes = requested.size ? italianRecipeDrafts.filter(recipe => requested.has(recipe.id)) : italianRecipeDrafts;
if (requested.size && recipes.length !== requested.size) throw new Error('Unknown Italian recipe id');
...
for (const recipe of recipes) {
  ...
  const sheetsRoot = resolve(root, 'assets', 'recipes', 'illustration-sheets-generated', 'italian');
  ...
  sets.push({
    recipeId: recipe.id,
    promptSet: 'italian-cooking-steps-v1',
...
if (requested.size) {
  const old = (await import('../src/italian-illustration-sets.mjs')).italianIllustrationSets;
  const updated = new Map(sets.map(set => [set.recipeId, set]));
  sets.splice(0, sets.length, ...old.map(set => updated.get(set.recipeId) || set));
}

await writeFile(join(root, 'src', 'italian-illustration-sets.mjs'), `// Generated from visually reviewed hand-free Italian source sheets and recorded panel layouts.\nexport const italianIllustrationSets = ${JSON.stringify(sets, null, 2)};\n`, 'utf8');
console.log(`Italian illustration manifest: ${sets.length} sets, ${sets.reduce((sum, set) => sum + set.hashes.length, 0)} separate step images.`);
