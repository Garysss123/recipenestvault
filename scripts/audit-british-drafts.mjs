...
const photos = contentOnly ? [] : (await import('../src/british-photos.mjs')).britishPhotoCandidates;
...
const promptSet = "british-cooking-steps-v1";
const generatedAt = "2026-09-14";
const hash = (buffer) => createHash("sha256").update(buffer).digest("hex");
const promptRecords = contentOnly ? [] : [JSON.parse(await readFile(resolve(root, 'docs/illustration-prompts/british-prompts.json'), 'utf8'))];
...
if (requireComplete && recipes.length !== 13) failures.push(`Expected exactly 13 British recipes; found ${recipes.length}`);
if (recipeIds.size !== recipes.length) failures.push("Duplicate Spanish recipe IDs");
if (!contentOnly && photosById.size !== photos.length) failures.push("Duplicate British photo IDs");
if (!contentOnly && (photos.length !== recipes.length || photos.some((photo) => !recipeIds.has(photo.id)))) failures.push("British recipe/photo identity mismatch");
...
for (const recipe of recipes) {
  if (recipe.cuisine !== "british") failures.push(`${recipe.id}: incorrect cuisine`);
...
if (contentOnly) {
  ...
  console.log('British content check passed: ' + recipes.length + ' current drafts; five-language fields, sources, method scenes and measurable facts verified. Publication assets are not checked in this mode.');
  process.exit(0);
}
...
let illustrations = [];
try {
  sets = (await import("../src/british-illustration-sets.mjs")).britishIllustrationSets;
...
if (sets.length !== recipes.length || setsById.size !== sets.length || sets.some((set) => !recipeIds.has(set.recipeId))) failures.push("British illustration-set identity mismatch");
...
if (new Set(illustrations.map((entry) => entry.sourceAsset)).size !== illustrations.length || new Set(illustrations.map((entry) => entry.sourceAssetSha256)).size !== illustrations.length) failures.push("A British step illustration is reused");
...
if (failures.length) {
  console.error(`British draft audit failed (${failures.length}):\n- ${failures.join("\n- ")}`);
  process.exit(1);
}
console.log(`British draft audit passed: ${recipes.length} five-language recipes, ${photos.length} licensed real photographs, ${expectedSteps} reviewed hand-free illustrations; content facts, mappings and hashes verified.`);
