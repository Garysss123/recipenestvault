...
import { britishRecipeDrafts } from '../src/british-recipes.mjs';

const root = dirname(dirname(fileURLToPath(import.meta.url)));
const promptFile = join(root, 'docs', 'illustration-prompts', 'british-prompts.json');
const record = JSON.parse(await readFile(promptFile, 'utf8'));
const entries = record.entries;
const byId = new Map(entries.map(entry => [entry.recipeId, entry]));
if (entries.length !== britishRecipeDrafts.length || byId.size !== entries.length) throw new Error('British prompt/recipe identity count mismatch');

const requested = new Set(process.argv.slice(2));
const recipes = requested.size ? britishRecipeDrafts.filter(recipe => requested.has(recipe.id)) : britishRecipeDrafts;
if (requested.size && recipes.length !== requested.size) throw new Error('Unknown British recipe id');
...
for (const recipe of recipes) {
  ...
  const sheetsRoot = resolve(root, 'assets', 'recipes', 'illustration-sheets-generated', 'british');
  ...
  sets.push({
    recipeId: recipe.id,
    promptSet: 'british-cooking-steps-v1',
...
if (requested.size) {
  let old = [];
  try {
    old = (await import('../src/british-illustration-sets.mjs')).britishIllustrationSets;
  } catch {
    old = [];
  }
...
await writeFile(join(root, 'src', 'british-illustration-sets.mjs'), `// Generated from visually reviewed hand-free British source sheets and recorded panel layouts.\nexport const britishIllustrationSets = ${JSON.stringify(sets, null, 2)};\n`, 'utf8');
console.log(`British illustration manifest: ${sets.length} sets, ${sets.reduce((sum, set) => sum + set.hashes.length, 0)} separate step images.`);
