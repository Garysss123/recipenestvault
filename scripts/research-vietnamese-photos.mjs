...
const dir = join(root, 'assets/recipes/alternatives/vietnamese');
await mkdir(dir, {recursive:true});
const [command, id, ...words] = process.argv.slice(2);
const headers = {'User-Agent':'RecipeNestVault/0.1 (Vietnamese licensed photo research; recipenestvault.com)'};
const api = new URL('https://commons.wikimedia.org/w/api.php');
const accepted = /^(?:CC0|Public domain|CC BY(?:-SA)?)(?:\s|$)/i;
async function get(url) { const r=await fetch(url,{headers}); if(!r.ok) throw new Error(`HTTP ${r.status}: ${url}`); return r; }
if(command==='search') {
 api.search=new URLSearchParams({action:'query',format:'json',formatversion:'2',generator:'search',gsrsearch:words.join(' '),gsrnamespace:'6',gsrlimit:'30',prop:'imageinfo',iiprop:'url|size|extmetadata'});
...
} else throw new Error('Usage: research-vietnamese-photos.mjs search|download id query|File:title');
