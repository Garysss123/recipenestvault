...
import { canadianRecipeDrafts } from '../src/canadian-recipes.mjs';

const root = dirname(dirname(fileURLToPath(import.meta.url)));
const promptFile = join(root, 'docs', 'illustration-prompts', 'canadian-prompts.json');
const record = JSON.parse(await readFile(promptFile, 'utf8'));
const byId = new Map(record.entries.map((entry) => [entry.recipeId, entry]));
if (record.entries.length !== canadianRecipeDrafts.length || byId.size !== record.entries.length) throw new Error('Canadian prompt/recipe identity mismatch');
const requested = new Set(process.argv.slice(2));
const recipes = requested.size ? canadianRecipeDrafts.filter((recipe) => requested.has(recipe.id)) : canadianRecipeDrafts;
if (requested.size && recipes.length !== requested.size) throw new Error('Unknown Canadian recipe id');
...
for (const recipe of recipes) {
  ...
  const sheetsRoot = resolve(root, 'assets', 'recipes', 'illustration-sheets-generated', 'canadian');
  ...
  sets.push({
    recipeId: recipe.id,
    promptSet: 'canadian-cooking-steps-v1',
...
let old = [];
try { old = (await import('../src/canadian-illustration-sets.mjs')).canadianIllustrationSets; } catch {}
...
await writeFile(join(root, 'src', 'canadian-illustration-sets.mjs'), `// Generated from visually reviewed Canadian ink-and-gouache source sheets and recorded panel layouts.\nexport const canadianIllustrationSets = ${JSON.stringify(merged, null, 2)};\n`, 'utf8');
console.log(`Canadian illustration manifest: ${merged.length} sets, ${merged.reduce((sum, set) => sum + set.hashes.length, 0)} distinct method illustrations.`);
