...
const dir = join(root, 'assets/recipes/alternatives/spanish');
await mkdir(dir, {recursive:true});
const [command, id, ...words] = process.argv.slice(2);
const headers = {'User-Agent':'RecipeNestVault/0.1 (Spanish licensed photo research; recipenestvault.com)'};
...
async function get(url) {
  for (let attempt = 0; attempt < 5; attempt += 1) {
    ...
    if (r.ok) return r;
    if (r.status !== 429 || attempt === 4) throw new Error(`HTTP ${r.status}: ${url}`);
    const retryAfter = Number(r.headers.get('retry-after')) || (3 * (attempt + 1));
...
} else throw new Error('Usage: research-spanish-photos.mjs search|download id query|File:title');
