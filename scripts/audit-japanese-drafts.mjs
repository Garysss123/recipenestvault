...
import { japanesePhotoCandidates } from "../src/japanese-photos.mjs";
import { japaneseRecipeDrafts } from "../src/japanese-recipes.mjs";

const root = dirname(dirname(fileURLToPath(import.meta.url)));
const localeOrder = ["en", "zh-hant", "ja", "ko", "th"];
const recipes = japaneseRecipeDrafts;
const photos = japanesePhotoCandidates;
...
if (new Set(recipes.map((recipe) => recipe.id)).size !== recipes.length) failures.push("duplicate Japanese recipe id");
if (new Set(photos.map((photo) => photo.id)).size !== photos.length) failures.push("duplicate Japanese photo id");
if (recipes.length !== photos.length) failures.push(`recipe/photo count mismatch: ${recipes.length}/${photos.length}`);

for (const recipe of recipes) {
  if (recipe.cuisine !== "japanese") failures.push(`${recipe.id}: wrong cuisine`);
  ...
  recipe.instructions.forEach((step, index) => {
    validateLocalized(step.title, `${recipe.id}.instructions[${index}].title`);
    validateLocalized(step.body, `${recipe.id}.instructions[${index}].body`);
    validateTemperatures(step.body, `${recipe.id}.instructions[${index}].body`);
  });
  for (const field of ["tips", "commonMistakes", "substitutions"]) recipe[field].forEach((row, index) => validateLocalized(row, `${recipe.id}.${field}[${index}]`));
...
for (const photo of photos) if (!recipes.some((recipe) => recipe.id === photo.id)) failures.push(`${photo.id}: orphan Japanese photo manifest`);

if (failures.length) {
  console.error(`Japanese draft audit failed (${failures.length}):\n- ${failures.join("\n- ")}`);
  process.exit(1);
}
console.log(`Japanese draft audit passed: ${recipes.length} recipes, ${photos.length} licensed real photographs, five locales and pinned hashes.`);
