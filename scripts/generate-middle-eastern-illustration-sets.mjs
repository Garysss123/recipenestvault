...
import { middleEasternRecipeDrafts } from '../src/middle-eastern-recipes.mjs';

const root = dirname(dirname(fileURLToPath(import.meta.url)));
const promptFile = join(root, 'docs', 'illustration-prompts', 'middle-eastern-prompts.json');
const record = JSON.parse(await readFile(promptFile, 'utf8'));
const byId = new Map(record.entries.map((entry) => [entry.recipeId, entry]));
if (record.entries.length !== middleEasternRecipeDrafts.length || byId.size !== record.entries.length) throw new Error('Middle Eastern prompt/recipe identity mismatch');
const requested = new Set(process.argv.slice(2));
const recipes = requested.size ? middleEasternRecipeDrafts.filter((recipe) => requested.has(recipe.id)) : middleEasternRecipeDrafts;
if (requested.size && recipes.length !== requested.size) throw new Error('Unknown Middle Eastern recipe id');
const hash = (buffer) => createHash('sha256').update(buffer).digest('hex');
const sets = [];

for (const recipe of recipes) {
  const entry = byId.get(recipe.id);
  if (!entry?.reviewed || !entry.prompt || entry.usedPanels !== recipe.instructions.length) throw new Error(`${recipe.id}: missing reviewed prompt and exact step mapping`);
  ...
  const sheetsRoot = resolve(root, 'assets', 'recipes', 'illustration-sheets-generated', 'middle-eastern');
  ...
  if ((metadata.width || 0) < 1200 || (metadata.height || 0) < 900) throw new Error(`${recipe.id}: source sheet is below minimum resolution`);
  const cropGrid = entry.cropGrid;
  if (!cropGrid || cropGrid.width !== metadata.width || cropGrid.height !== metadata.height) throw new Error(`${recipe.id}: missing visually reviewed pixel grid`);
  ...
  sets.push({
    recipeId: recipe.id,
    promptSet: 'middle-eastern-cooking-steps-v1',
  ...
  console.log(`Prepared ${recipe.id}: ${hashes.length} hand-illustrated steps`);
}

let old = [];
try { old = (await import('../src/middle-eastern-illustration-sets.mjs')).middleEasternIllustrationSets; } catch {}
...
await writeFile(join(root, 'src', 'middle-eastern-illustration-sets.mjs'), `// Generated from visually reviewed Middle Eastern ink-and-gouache source sheets and recorded panel layouts.\nexport const middleEasternIllustrationSets = ${JSON.stringify(merged, null, 2)};\n`, 'utf8');
console.log(`Middle Eastern illustration manifest: ${merged.length} sets, ${merged.reduce((sum, set) => sum + set.hashes.length, 0)} distinct method illustrations.`);
