...
import { brazilianRecipeDrafts as recipes } from "../src/brazilian-recipes.mjs";

const contentOnly = process.argv.includes('--content-only');
const requireComplete = process.argv.includes('--complete');
const photos = contentOnly ? [] : (await import('../src/brazilian-photos.mjs')).brazilianPhotoCandidates;
...
const promptSet = "brazilian-cooking-steps-v1";
const generatedAt = "2026-09-15";
const hash = (buffer) => createHash("sha256").update(buffer).digest("hex");
const promptRecords = contentOnly ? [] : [JSON.parse(await readFile(resolve(root, 'docs/illustration-prompts/brazilian-prompts.json'), 'utf8'))];
...
if (requireComplete && recipes.length !== 13) failures.push(`Expected exactly 13 Brazilian recipes; found ${recipes.length}`);
if (recipeIds.size !== recipes.length) failures.push("Duplicate Brazilian recipe IDs");
if (!contentOnly && photosById.size !== photos.length) failures.push("Duplicate Brazilian photo IDs");
if (!contentOnly && (photos.length !== recipes.length || photos.some((photo) => !recipeIds.has(photo.id)))) failures.push("Brazilian recipe/photo identity mismatch");
if (!contentOnly && new Set(photos.map((photo) => photo.sourceAssetSha256)).size !== photos.length) failures.push("A finished photograph is reused across Brazilian recipes");
...
for (const recipe of recipes) {
  if (recipe.cuisine !== "brazilian") failures.push(`${recipe.id}: incorrect cuisine`);
...
if (contentOnly) {
  ...
  console.log('Brazilian content check passed: ' + recipes.length + ' current drafts; five-language fields, sources, method scenes and measurable facts verified. Publication assets are not checked in this mode.');
  process.exit(0);
}
...
let illustrations = [];
try {
  sets = (await import("../src/brazilian-illustration-sets.mjs")).brazilianIllustrationSets;
...
if (sets.length !== recipes.length || setsById.size !== sets.length || sets.some((set) => !recipeIds.has(set.recipeId))) failures.push("Brazilian illustration-set identity mismatch");
...
if (new Set(illustrations.map((entry) => entry.sourceAsset)).size !== illustrations.length || new Set(illustrations.map((entry) => entry.sourceAssetSha256)).size !== illustrations.length) failures.push("A Brazilian step illustration is reused");
...
if (failures.length) {
  console.error(`Brazilian draft audit failed (${failures.length}):\n- ${failures.join("\n- ")}`);
  process.exit(1);
}
console.log(`Brazilian draft audit passed: ${recipes.length} five-language recipes, ${photos.length} licensed real photographs, ${expectedSteps} reviewed hand-free illustrations; content facts, mappings and hashes verified.`);
