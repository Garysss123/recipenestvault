...
import { thaiPhotoCandidates } from "../src/thai-photos.mjs";
...
const recipes = thaiRecipeDrafts;
const photos = thaiPhotoCandidates;
...
function validateLocalized(value, label) {
  for (const locale of localeOrder) {
    if (typeof value?.[locale] !== "string" || !value[locale].trim()) failures.push(`${label}.${locale}: missing localized text`);
  }
}
...
function validateTemperatures(value, label) {
  ...
  for (const locale of localeOrder) {
    if (temperatureFacts(value?.[locale]) !== expected) failures.push(`${label}.${locale}: temperature facts differ from English`);
  }
}

if (recipeIds.size !== recipes.length) failures.push("duplicate Thai recipe id");
if (new Set(photos.map((photo) => photo.id)).size !== photos.length) failures.push("duplicate Thai photo id");
if (recipes.length < 20) failures.push(`Thai first edition has ${recipes.length} recipes; at least 20 are required`);
...
for (const recipe of recipes) {
  if (recipe.cuisine !== "thai") failures.push(`${recipe.id}: wrong cuisine`);
  ...
  for (const source of recipe.sources) {
    if (!source.title || !/^https:\/\//.test(source.url || "")) failures.push(`${recipe.id}: invalid recipe source`);
  }

  const photo = photosById.get(recipe.id);
  if (!photo) {
    failures.push(`${recipe.id}: missing finished photograph manifest`);
    continue;
  }
...
for (const photo of photos) if (!recipeIds.has(photo.id)) failures.push(`${photo.id}: orphan Thai photo manifest`);

const expectedIllustrationCount = recipes.reduce((sum, recipe) => sum + recipe.instructions.length, 0);
if (illustrations.length !== expectedIllustrationCount) failures.push(`Thai illustration count mismatch: expected ${expectedIllustrationCount}, found ${illustrations.length}`);
...
for (const recipe of recipes) {
  ...
  for (const illustration of set) {
    ...
    if (illustrationAssets.has(illustration.sourceAsset)) failures.push(`${illustration.sourceAsset}: illustration source reused by more than one Thai step`);
    ...
    if (illustration.generator !== "OpenAI image_gen" || illustration.generatedAt !== "2026-08-31" || illustration.promptSet !== "thai-cooking-steps-v1") failures.push(`${illustration.id}: Thai illustration provenance changed`);
...
if (failures.length) {
  console.error(`Thai draft audit failed (${failures.length}):\n- ${failures.join("\n- ")}`);
  process.exit(1);
}

console.log(`Thai draft audit passed: ${recipes.length} recipes, natural ${Math.min(...stepCounts)}–${Math.max(...stepCounts)} step methods, ${photos.length} licensed real photographs, ${illustrations.length} reviewed step illustrations, five locales and pinned hashes.`);
