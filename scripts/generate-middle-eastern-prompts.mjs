...
const entries = middleEasternRecipeDrafts.map((recipe) => {
  const columns = 3;
  const rows = 3;
  ...
  return {
    ...
    style,
    prompt,
    sourceSheet: `assets/recipes/illustration-sheets-generated/middle-eastern/${recipe.id}.png`,
    reviewed: false
  };
});
if (entries.length !== 13 || new Set(entries.map((entry) => entry.recipeId)).size !== 13 || entries.some((entry) => entry.usedPanels > 9)) throw new Error('Middle Eastern illustration prompt identity/grid mismatch');
const record = { promptSet, generatedAt: '2026-09-19', style, entries };
const jsonPath = join(root, 'docs', 'illustration-prompts', 'middle-eastern-prompts.json');
await mkdir(dirname(jsonPath), { recursive: true });
await writeFile(jsonPath, JSON.stringify(record, null, 2) + '\n', 'utf8');
const markdown = [
  '# Middle Eastern recipe step illustration sheets',
  '',
...
const markdownPath = join(root, 'docs', 'illustration-prompts', 'middle-eastern-cooking-steps-v1.md');
await writeFile(markdownPath, markdown, 'utf8');
console.log(`Wrote ${relative(root, jsonPath)} and ${relative(root, markdownPath)} for ${entries.length} Middle Eastern recipe sheets.`);
