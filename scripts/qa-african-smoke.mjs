...
const outputDir = join(root, 'qa-artifacts', 'visual', `${new URL(baseUrl).hostname}-african-smoke-${new Date().toISOString().replace(/[:.]/g, '-')}`);
...
await inspect({
  name: 'en-african-collection-desktop', path: '/en/cuisines/african/', viewport: { width: 1440, height: 1000 },
  check: async (page) => {
    ...
    if (!(await page.locator('body').evaluate((element) => element.classList.contains('cuisine-african')))) throw new Error('Collection cuisine theme class is missing');
...
await inspect({
  name: 'zh-hant-african-collection-mobile', path: '/zh-hant/cuisines/african/', viewport: { width: 390, height: 844 },
  check: async (page) => {
...
await inspect({
  name: 'en-nigerian-jollof-photo-desktop', path: '/en/recipes/nigerian-jollof-rice/', viewport: { width: 1440, height: 1000 }, fullPage: false,
  check: async (page) => {
    const photo = page.locator('.recipe-detail-photo img');
    if (!(await photo.evaluate((image) => image.complete && image.naturalWidth > 0))) throw new Error('Jollof rice finished-dish photo failed to load');
    const steps = page.locator('.method-section ol > li');
    if (await steps.count() !== 6 || await page.locator('.recipe-step-illustration').count() !== 6) throw new Error('Jollof rice method does not have six matching step illustrations');
    ...
    if (broken) throw new Error(`${broken} Jollof rice step illustrations failed to load`);
  }
});

await inspect({
  name: 'zh-hant-kelewele-mobile', path: '/zh-hant/recipes/kelewele/', viewport: { width: 390, height: 844 }, fullPage: false,
  check: async (page) => {
    if (await page.locator('.method-section ol > li').count() !== 5) throw new Error('Kelewele method does not show five steps');
    if (await page.locator('.recipe-step-illustration img').evaluateAll((images) => images.filter((image) => !image.complete || !image.naturalWidth).length)) throw new Error('A kelewele step image failed to load');
  }
});
...
console.log(`African smoke QA: ${results.length} states from ${baseUrl}; evidence in ${outputDir}`);
if (failures.length) {
  console.error(`African smoke QA failed (${failures.length}):\n- ${failures.join('\n- ')}`);
  process.exit(1);
}
console.log('African desktop/mobile collection, finished-photo, step-art, route, network and overflow checks passed.');
