...
const file=new URL('../docs/illustration-prompts/italian-prompts.json',import.meta.url);
const record=JSON.parse(await readFile(file,'utf8'));
const byId=new Map(record.entries.map(entry=>[entry.recipeId,entry]));
for(const recipe of italianRecipeDrafts){
...
console.log(`Italian prompts: ${record.entries.length} source sheets, ${record.entries.reduce((n,e)=>n+e.usedPanels,0)} planned step images.`);
