...
import { spanishRecipeDrafts } from '../src/spanish-recipes.mjs';

const root = dirname(dirname(fileURLToPath(import.meta.url)));
const promptFile = join(root, 'docs', 'illustration-prompts', 'spanish-prompts.json');
const record = JSON.parse(await readFile(promptFile, 'utf8'));
const entries = record.entries;
const byId = new Map(entries.map(entry => [entry.recipeId, entry]));
if (entries.length !== spanishRecipeDrafts.length || byId.size !== entries.length) throw new Error('Spanish prompt/recipe identity count mismatch');

const requested = new Set(process.argv.slice(2));
const recipes = requested.size ? spanishRecipeDrafts.filter(recipe => requested.has(recipe.id)) : spanishRecipeDrafts;
if (requested.size && recipes.length !== requested.size) throw new Error('Unknown Spanish recipe id');
...
for (const recipe of recipes) {
  ...
  const sheetsRoot = resolve(root, 'assets', 'recipes', 'illustration-sheets-generated', 'spanish');
...
if (requested.size) {
  let old = [];
  try {
    old = (await import('../src/spanish-illustration-sets.mjs')).spanishIllustrationSets;
  } catch {
    old = [];
  }
...
await writeFile(join(root, 'src', 'spanish-illustration-sets.mjs'), `// Generated from visually reviewed hand-free Spanish source sheets and recorded panel layouts.\nexport const spanishIllustrationSets = ${JSON.stringify(sets, null, 2)};\n`, 'utf8');
console.log(`Spanish illustration manifest: ${sets.length} sets, ${sets.reduce((sum, set) => sum + set.hashes.length, 0)} separate step images.`);
