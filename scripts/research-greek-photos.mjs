...
const dir = join(root, 'assets/recipes/alternatives/greek');
await mkdir(dir, { recursive: true });

const [command, id, ...words] = process.argv.slice(2);
const headers = { 'User-Agent': 'RecipeNestVault/0.1 (Greek licensed photo research; recipenestvault.com)' };
...
if (command === 'search') {
  ...
  throw new Error('Usage: research-greek-photos.mjs search|download id query|File:title');
}
