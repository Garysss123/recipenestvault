...
import { middleEasternPhotoCandidates } from '../src/middle-eastern-photos.mjs';
...
const cuisineIds = new Set(middleEasternRecipeDrafts.map((recipe) => recipe.id));
const photos = middleEasternPhotoCandidates.filter((photo) => cuisineIds.has(photo.id) && photo.visualMatchApproved && photo.commercialUseVerified && photo.realPhoto);
...
if (photos.length !== middleEasternRecipeDrafts.length || illustrations.length !== middleEasternRecipeDrafts.reduce((sum, recipe) => sum + recipe.instructions.length, 0)) throw new Error('Middle Eastern photo or step-illustration identity/count mismatch');
...
console.log(`Generated ${photos.length} Middle Eastern photo sets and ${illustrations.length} illustrated method steps without touching other cuisine assets.`);
