...
function renderRecipe(slug, recipe) {
  ...
  const instructionRows = recipe.instructions.map((step, index) => {
    ...
    return `<li><span>${String(index + 1).padStart(2, "0")}</span><div class="recipe-step-copy">${instruction.title ? `<h3>${esc(instruction.title)}</h3>` : ""}<p>${esc(instruction.body)}</p></div></li>`;
  }).join("");
