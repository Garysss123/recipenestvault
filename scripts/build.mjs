...
function renderRecipe(slug, recipe) {
  ...
  const ingredientRows = recipe.ingredients.map((row) => `<li><span>${esc(row.amount)}</span><p>${esc(localized(row.item, slug))}</p></li>`).join("");
  const instructionRows = recipe.instructions.map((step, index) => `<li><span>${String(index + 1).padStart(2, "0")}</span><p>${esc(cleanStep(localized(step, slug)))}</p></li>`).join("");
