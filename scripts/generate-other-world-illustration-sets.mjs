...
import { otherWorldRecipeDrafts } from '../src/other-world-recipes.mjs';

const root = dirname(dirname(fileURLToPath(import.meta.url)));
const promptFile = join(root, 'docs', 'illustration-prompts', 'other-world-prompts.json');
const record = JSON.parse(await readFile(promptFile, 'utf8'));
const byId = new Map(record.entries.map((entry) => [entry.recipeId, entry]));
if (record.entries.length !== otherWorldRecipeDrafts.length || byId.size !== record.entries.length) throw new Error('Other-world prompt/recipe identity mismatch');
const requested = new Set(process.argv.slice(2));
const recipes = requested.size ? otherWorldRecipeDrafts.filter((recipe) => requested.has(recipe.id)) : otherWorldRecipeDrafts;
if (requested.size && recipes.length !== requested.size) throw new Error('Unknown Other-world recipe id');
...
for (const recipe of recipes) {
  ...
  const sheetsRoot = resolve(root, 'assets', 'recipes', 'illustration-sheets-generated', 'other-world');
  ...
  sets.push({
    recipeId: recipe.id,
    promptSet: 'other-world-cooking-steps-v1',
...
let old = [];
try { old = (await import('../src/other-world-illustration-sets.mjs')).otherWorldIllustrationSets; } catch {}
...
await writeFile(join(root, 'src', 'other-world-illustration-sets.mjs'), `// Generated from visually reviewed Other-world ink-and-gouache source sheets and recorded panel layouts.\nexport const otherWorldIllustrationSets = ${JSON.stringify(merged, null, 2)};\n`, 'utf8');
console.log(`Other-world illustration manifest: ${merged.length} sets, ${merged.reduce((sum, set) => sum + set.hashes.length, 0)} distinct method illustrations.`);
