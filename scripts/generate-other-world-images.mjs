...
import { otherWorldPhotoCandidates } from '../src/other-world-photos.mjs';
...
const cuisineIds = new Set(otherWorldRecipeDrafts.map((recipe) => recipe.id));
const photos = otherWorldPhotoCandidates.filter((photo) => cuisineIds.has(photo.id) && photo.visualMatchApproved && photo.commercialUseVerified && photo.realPhoto);
...
if (photos.length !== otherWorldRecipeDrafts.length || illustrations.length !== otherWorldRecipeDrafts.reduce((sum, recipe) => sum + recipe.instructions.length, 0)) throw new Error('Other-world photo or step-illustration identity/count mismatch');
...
console.log(`Generated ${photos.length} Other-world photo sets and ${illustrations.length} illustrated method steps without touching other cuisine assets.`);
