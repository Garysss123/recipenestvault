...
import { mediterraneanPhotoCandidates } from '../src/mediterranean-photos.mjs';
...
const cuisineIds = new Set(mediterraneanRecipeDrafts.map((recipe) => recipe.id));
const photos = mediterraneanPhotoCandidates.filter((photo) => cuisineIds.has(photo.id) && photo.visualMatchApproved && photo.commercialUseVerified && photo.realPhoto);
...
if (photos.length !== mediterraneanRecipeDrafts.length || illustrations.length !== mediterraneanRecipeDrafts.reduce((sum, recipe) => sum + recipe.instructions.length, 0)) throw new Error('Mediterranean photo or step-illustration identity/count mismatch');
...
console.log(`Generated ${photos.length} Mediterranean photo sets and ${illustrations.length} illustrated method steps without touching other cuisine assets.`);
