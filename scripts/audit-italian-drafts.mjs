...
import { italianRecipeDrafts as recipes } from "../src/italian-recipes.mjs";

const contentOnly = process.argv.includes('--content-only');
const photos = contentOnly ? [] : (await import('../src/italian-photos.mjs')).italianPhotoCandidates;
...
const promptSet = "italian-cooking-steps-v1";
const generatedAt = "2026-09-13";
const hash = (buffer) => createHash("sha256").update(buffer).digest("hex");
const promptRecords = contentOnly ? [] : [JSON.parse(await readFile(resolve(root, 'docs/illustration-prompts/italian-prompts.json'), 'utf8'))];
...
if (recipes.length < 21) failures.push(`Expected more than 20 Italian recipes; found ${recipes.length}`);
if (recipeIds.size !== recipes.length) failures.push("Duplicate Italian recipe IDs");
if (!contentOnly && photosById.size !== photos.length) failures.push("Duplicate Italian photo IDs");
if (!contentOnly && (photos.length !== recipes.length || photos.some((photo) => !recipeIds.has(photo.id)))) failures.push("Italian recipe/photo identity mismatch");
if (!contentOnly && new Set(photos.map((photo) => photo.sourceAssetSha256)).size !== photos.length) failures.push("A finished photograph is reused across Italian recipes");
const stepCounts = recipes.map((recipe) => recipe.instructions?.length || 0);
if (!contentOnly && new Set(stepCounts).size < 3) failures.push("Methods have lost their natural step-count variation");

for (const recipe of recipes) {
  if (recipe.cuisine !== "italian") failures.push(`${recipe.id}: incorrect cuisine`);
  ...
  if (recipe.ingredients.length < 6 || recipe.instructions.length < 4) failures.push(`${recipe.id}: incomplete ingredients or actionable method`);
...
if (contentOnly) {
  ...
  console.log('Italian content check passed: ' + recipes.length + ' current drafts; all five-language fields and measurable facts verified. Publication assets are not checked in this mode.');
  process.exit(0);
}
...
let illustrations = [];
try {
  sets = (await import("../src/italian-illustration-sets.mjs")).italianIllustrationSets;
...
if (sets.length !== recipes.length || setsById.size !== sets.length || sets.some((set) => !recipeIds.has(set.recipeId))) failures.push("Italian illustration-set identity mismatch");
...
if (new Set(illustrations.map((entry) => entry.sourceAsset)).size !== illustrations.length || new Set(illustrations.map((entry) => entry.sourceAssetSha256)).size !== illustrations.length) failures.push("A Italian step illustration is reused");
...
if (failures.length) {
  console.error(`Italian draft audit failed (${failures.length}):\n- ${failures.join("\n- ")}`);
  process.exit(1);
}
console.log(`Italian draft audit passed: ${recipes.length} five-language recipes, ${photos.length} licensed real photographs, ${expectedSteps} reviewed hand-free illustrations; content facts, mappings and hashes verified.`);
