...
import { mexicanRecipeDrafts } from '../src/mexican-recipes.mjs';

const root = dirname(dirname(fileURLToPath(import.meta.url)));
const promptFile = join(root, 'docs', 'illustration-prompts', 'mexican-prompts.json');
const record = JSON.parse(await readFile(promptFile, 'utf8'));
const entries = record.entries;
const byId = new Map(entries.map(entry => [entry.recipeId, entry]));
if (entries.length !== mexicanRecipeDrafts.length || byId.size !== entries.length) throw new Error('Mexican prompt/recipe identity count mismatch');

const requested = new Set(process.argv.slice(2));
const recipes = requested.size ? mexicanRecipeDrafts.filter(recipe => requested.has(recipe.id)) : mexicanRecipeDrafts;
if (requested.size && recipes.length !== requested.size) throw new Error('Unknown Mexican recipe id');
...
for (const recipe of recipes) {
  ...
  const sheetsRoot = resolve(root, 'assets', 'recipes', 'illustration-sheets-generated', 'mexican');
  ...
  sets.push({
    recipeId: recipe.id,
    promptSet: 'mexican-cooking-steps-v1',
...
if (requested.size) {
  let old = [];
  try {
    old = (await import('../src/mexican-illustration-sets.mjs')).mexicanIllustrationSets;
  } catch {
    old = [];
  }
...
await writeFile(join(root, 'src', 'mexican-illustration-sets.mjs'), `// Generated from visually reviewed hand-free Mexican source sheets and recorded panel layouts.\nexport const mexicanIllustrationSets = ${JSON.stringify(sets, null, 2)};\n`, 'utf8');
console.log(`Mexican illustration manifest: ${sets.length} sets, ${sets.reduce((sum, set) => sum + set.hashes.length, 0)} separate step images.`);
